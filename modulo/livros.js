...
const getTitle = (titulo) => {
    let nomeLivro = titulo.toLowerCase()
    let books = []
    let totalLivros = {}
    let erro = true

    if (typeof(nomeLivro) != `undefined`) {
        if (nomeLivro != "") {
            //percorre o array principal
            livros.forEach (item => {
                //percorre o elemento books dentro de livros
                item.books.forEach (item2 => {
                    //localiza a palavra chave  dentro do titulo do livro usando IndexOf
                    if (item2.title.toLowerCase().indexOf(nomeLivro) != -1) {
                    //Adciona no array um JSON com todos os elementos solicitados
                        books.push ({
                            Name: item2.title,
                ...
                totalLivros.Total = books.length
                totalLivros.Books = books
            })
        }
    }
    if (erro) {
